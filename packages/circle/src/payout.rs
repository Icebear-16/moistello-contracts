...
pub fn resolve_vote(env:&Env,circle:&Circle,round:u32)->Result<Address,CircleError>{
    ...
    for(addr,count)in tally.iter(){
        if count>best_count && active_positions.get(addr.clone()).is_some(){
            best_count=count;
            best_addr=Some(addr);
        }
    }
    let winner=best_addr.ok_or(CircleError::NotMember)?;
    let position=active_positions.get(winner.clone()).ok_or(CircleError::NotMember)?;
    if(circle.payout_bitmap&(1u128<<position))!=0{
        return Err(CircleError::PayoutAlreadyExecuted);
    }
    Ok(winner)
}
